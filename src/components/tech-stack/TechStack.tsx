...
import { ExpandedSkillPanel } from './ExpandedSkillPanel';

export function TechStack() {
  const [expandedSkill, setExpandedSkill] = useState<Skill | null>(null);

  return (
    <section id="tech-stack" style={{ width: '100vw', marginLeft: 'calc(-50vw + 50%)', position: 'relative', padding: '96px 0', background: 'transparent', overflow: 'hidden' }}>
      ...
      <div style={{ position: 'relative', width: '100%', overflow: 'hidden', padding: '40px 0' }}>
        <motion.div style={{ display: 'flex', flexDirection: 'row', gap: '40px', alignItems: 'center', flexWrap: 'nowrap', width: 'max-content', paddingLeft: '24px' }} animate={{ x: [0, '-50%'] }} transition={{ repeat: Infinity, ease: 'linear', duration: 45 }}>
          {[...skillCategories.flatMap(c => c.skills), ...skillCategories.flatMap(c => c.skills)].map((skill, idx) => (
            <SkillCard key={`${skill.id}-${idx}`} skill={skill} onClick={() => setExpandedSkill(skill)} />
          ))}
        </motion.div>
      ...
      <AnimatePresence>
        {expandedSkill && (
          <ExpandedSkillPanel skill={expandedSkill} onClose={() => setExpandedSkill(null)} />
        )}
      </AnimatePresence>
    </section>
  );
